package com.github.ag.core.rest;
...
public class ResilientRestClient {
	...
	public <T> ResponseEntity<T> exchange(String hystrixKey, URI url, HttpMethod method, HttpEntity<?> httpEntity,
			Class<T> type, T defaultResponse) {
	...
	public <T> ResponseEntity<T> exchange(String hystrixKey, URI url, HttpMethod method, HttpEntity<?> httpEntity,
			Class<T> type) throws RestCallException {
		log.debug("hystrixKey is {}", hystrixKey);
		CircuitBreaker circuitBreaker = getCircuitBreaker(hystrixKey);
		Map<String, String> contextMap = getContextMap();
		try {
	...
	private <T> T buildException(RuntimeException exception) throws RestCallException {
		...
		if (cause instanceof HttpClientErrorException) {
			...
			throw new RestCallException("Unauthorized Access", httpClientErrorException.getStatusCode(),
